...
# Title of the app
st.title("Burdah Memorization Helper Version 0 - test fix chapters - not work")
...
chapter = st.selectbox("Select Chapter", sorted(df['Chapter'].unique()))
...
start_time = filtered_df[filtered_df['Line Number'] == start_line]['Start Time'].values[0] * 1000  # Convert to milliseconds
end_time = filtered_df[filtered_df['Line Number'] == end_line]['End Time'].values[0] * 1000  # Convert to milliseconds
